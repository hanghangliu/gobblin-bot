...
func DigestTitle() string {
	...
	t := time.Now()
	t = t.In(loc)
	return fmt.Sprintf("Apache Gobblin Daily Email Digest (%s)", t.Format("2006-01-02"))
}
...
func SendGridEmail(c *Config, subject string, htmlContent string) string {
	from := mail.NewEmail("Gobblin Slack Email Digest", c.From)
	client := sendgrid.NewSendClient(c.SendgridToken)
	to := mail.NewEmail("Apache Gobblin Dev", c.To)
